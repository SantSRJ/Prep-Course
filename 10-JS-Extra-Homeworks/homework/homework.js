...
function asAmirror(str) {
  ...
  //Escribe tu código aquí
 var inverter = str.split(' ');
 var arr = [];
 for (var i = 0; i < inverter.length; i++) {
  arr.push(inverter[i].split('').reverse().join(''))
 }
 return arr.join(' ');
} 
...
function sortArray(arr) {
  ...
  //Escribe tu código aquí
  var nuevo_array = [];
  var indexs = arr.map(a => a.length);
  indexs = indexs.sort().filter((e, i) => indexs.indexOf(e) === i);
  indexs.forEach(a => arr.forEach(b => b.length === a && nuevo_array.push(b)));
  return nuevo_array;
}
